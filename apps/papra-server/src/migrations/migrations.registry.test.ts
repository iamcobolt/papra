  ...
  describe('migrations', () => {
    ...
    test('regression test of the database state after running migrations, update the snapshot when the database state changes', async () => {
      ...
      expect(await serializeSchema({ db })).toMatchInlineSnapshot(`
        ...
        CREATE UNIQUE INDEX document_semantic_chunks_document_chunk_unique ON document_semantic_chunks (document_id, chunk_index);
        CREATE INDEX document_semantic_chunks_document_id_index ON document_semantic_chunks (document_id);
        CREATE INDEX document_semantic_chunks_organization_document_index ON document_semantic_chunks (organization_id, document_id);
        ...
        CREATE TABLE document_semantic_chunks ( id text PRIMARY KEY NOT NULL, created_at integer NOT NULL, updated_at integer NOT NULL, organization_id text NOT NULL REFERENCES organizations(id) ON DELETE CASCADE ON UPDATE CASCADE, document_id text NOT NULL REFERENCES documents(id) ON DELETE CASCADE ON UPDATE CASCADE, chunk_index integer NOT NULL, content_hash text NOT NULL, embedding F32_BLOB(1536) NOT NULL );
        CREATE TABLE "documents" ( "id" text PRIMARY KEY NOT NULL, "created_at" integer NOT NULL, "updated_at" integer NOT NULL, "is_deleted" integer DEFAULT false NOT NULL, "deleted_at" integer, "organization_id" text NOT NULL, "created_by" text, "deleted_by" text, "original_name" text NOT NULL, "original_size" integer DEFAULT 0 NOT NULL, "original_storage_key" text NOT NULL, "original_sha256_hash" text NOT NULL, "name" text NOT NULL, "mime_type" text NOT NULL, "content" text DEFAULT '' NOT NULL, file_encryption_key_wrapped TEXT, file_encryption_kek_version TEXT, file_encryption_algorithm TEXT, document_date INTEGER, semantic_indexed_at INTEGER, semantic_index_content_hash TEXT, FOREIGN KEY ("organization_id") REFERENCES "organizations"("id") ON UPDATE cascade ON DELETE cascade, FOREIGN KEY ("created_by") REFERENCES "users"("id") ON UPDATE cascade ON DELETE set null, FOREIGN KEY ("deleted_by") REFERENCES "users"("id") ON UPDATE cascade ON DELETE set null );
        ...
        CREATE TABLE semantic_vector_meta ( id text PRIMARY KEY NOT NULL, schema_version integer NOT NULL, model text NOT NULL, dimensions integer NOT NULL, metric text NOT NULL, chunk_size integer NOT NULL, chunk_overlap integer NOT NULL, created_at integer NOT NULL, updated_at integer NOT NULL );
